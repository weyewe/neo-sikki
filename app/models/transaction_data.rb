class TransactionData < ActiveRecord::Base
  ...
  def delete_object 
    ...
    self.transaction_activity_datas.each {|x| x.delete_object }
    self.destroy  
  end
  ...
  def internal_delete_object
    ...
    self.transaction_activity_datas.each { |x| x.internal_delete_object}
    self.destroy 
  end
  ...
  def update_affected_accounts_due_to_confirmation
    self.transaction_activity_datas.each do |ta_entry|
  ...
  def confirm
    if self.transaction_activity_datas.count == 0 
  ...
  def total_debit
    self.transaction_activity_datas.where(:entry_case => NORMAL_BALANCE[:debit]).sum("amount")
  end
  
  def total_credit
    self.transaction_activity_datas.where(:entry_case => NORMAL_BALANCE[:credit]).sum("amount")
  end 
...
=end
  ...
  def create_initial_migration
    ...
    current_account_migration = TransactionDataEntry.create_object(
    ...
    temporary_account_migration = TransactionDataEntry.create_object(
      :transaction_activity_id =>  self.id,
